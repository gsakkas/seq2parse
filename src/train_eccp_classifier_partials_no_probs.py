...
if __name__ == "__main__":
    ...
    saved_model_file = join(outDir, 'models', 'transformer-classifier-partial-parses-no-probs.h5')
    ...
    if 'MANJARO' in release():
        ...
        for partPath in list(data_dir_2017.glob('part_*')):
            dataset_part_file = join(partPath, "erule-dataset-partials-no-probs-fixes.txt")
    ...
    tokens = dict()
    reverse_tokens = dict()
    tokens_file = join(outDir, "tokens_ints-partials-no-probs.json")
    rev_tokens_file = join(outDir, "tokens_rev_ints-partials-no-probs.json")
    ...
    labels = dict()
    reverse_labels = dict()
    labels_file = join(outDir, "erule_labels-partials-no-probs.json")
    rev_labels_file = join(outDir, "erule_reverse_labels-partials-no-probs.json")
    if exists(saved_model_file):
    ...
    if exists(saved_model_file):
        with open(join(outDir, "erules-partials-no-probs.json"), "r") as fin:
            erules_cnt = json.load(fin)
    else:
        for erls in ys:
            for er in erls:
                erules_cnt[er] += 1
        with open(join(outDir, "erules-partials-no-probs.json"), "w") as fout:
    ...
    if not exists(join(outDir, "erule-test-set-generic-no-probs.txt")):
        with open(join(outDir, "erule-test-set-generic-no-probs.txt"), "w") as outFile:
    ...
    if not exists(join(outDir, "test-set-50-popular-partials-no-probs.txt")):
        top_50_erules = [er[0] for er in all_erules[:50]]
        with open(join(outDir, "test-set-50-popular-partials-no-probs.txt"), "w") as outFile:
    ...
    try:
        ...
        with tf.device(gpuToUse):
            ...
            with open(join(outDir, "PartialParsesNoProbsClassifierStats.txt"), "w") as out_file:
            ...
            if do_time_tests:
                    ...
                    while True:
                        try:
                            ...
                            if bruh:
                                ...
                                if (failed + done) % 50 == 0:
                                    print_results(failed, done, parses_bad, avg_run_time, parsed_progs_times, total_size, time_gains, outDir, "trans_cls_results-partials-no-probs.txt", max_time=120)
                ...
                with open(join(outDir, "test-set-top-20-partials-no-probs.txt"), "w") as outFile:
                    with open(join(outDir, "test-set-best-partials-no-probs.txt"), "w") as outFileBest:
