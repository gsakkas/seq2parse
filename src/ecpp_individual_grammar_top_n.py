...
class ErrorGrammar(Grammar):
    ...
    def update_error_grammar(self, changes):
        ...
        for ch in changes:
            ...
            if ch[0] == 'deleted':
                for sym in important_tags:
                    ...
                    self.add(ErrorRule("Err_" + sym, ["H", sym]))
            ...
            if not key.startswith('Err_') and not key.startswith('InsertErr'):
                for rule in self.rules[key]:
                    if rule.error_score() < 1:
                        new_rule_1 = ErrorGrammar.error_rule_1(self, rule, all_important_tags)
        ...
        #third step
        self.add(Rule("S'", ["S"]))
        self.add(ErrorRule("S'", ["S", "InsertErr"]))
        self.add(ErrorRule("H", ["H", "InsertErr"]))
        self.add(ErrorRule("H", ["InsertErr"]))
        # print(self)
        # print(len(str(self).split('\n')))
...
class ChartEntry():
    ...
    def switch_states(self):
        ...
        self.states = self.other_states + self.states
        self.other_states = []
        # print(len(self.states), len(self.other_states))
    ...
    def add(self, state):
        ...
        if not in_states and not in_other_states and have_not_seen:
            ...
            same_states = [(i, x) for i, x in enumerate(self.states + self.other_states + self.seen) if x == state]
            if len(same_states) > 3:
                max_i, max_state = max(same_states, key = lambda x: x[1].error_count)
                # print("OK MISTER")
                if max_state.error_count > state.error_count:
                    # print("YOLO")
                    if max_i < len(self.states):
                        self.states.pop(max_i)
                    elif max_i < len(self.states) + len(self.other_states):
                        self.other_states.pop(max_i - len(self.states))
                    else:
                        pass
...
def rule_updates_rhs(grammar, st):
    if st.rule.lhs.startswith('Err_'):
        grammar[st.rule.lhs.replace('Err_', '')][0].rhs[0]
    elif st.rule.lhs == 'InsertErr':
        return None
    else:
...
def fixed_lexed_prog(lexed_prog, grammar):
    def run_parse(sentence):
        parser = ErrorEarleyParse(sentence, grammar)
        parser.parse()
    ...
    if parsed is None:
        ...
        return (None, None, None)
    abstract_fixed_seq = list(filter(lambda x: x, map(rule_updates, sorted(parsed, key=lambda st: st.sent_pos))))
    rul_upd = partial(rule_updates_rhs, grammar)
    fixed_seq = list(filter(lambda x: x, map(rul_upd, sorted(parsed, key=lambda st: st.sent_pos))))
    rul_upd_repair_ops = partial(rule_updates_repair_operations, grammar)
    fixed_seq_ops = list(filter(lambda x: x, map(rul_upd_repair_ops, sorted(parsed, key=lambda st: st.sent_pos))))
    # print(parsed)
    # print(' '.join(fixed_seq))
    # print('*' * 42)
    return (' '.join(abstract_fixed_seq), ' '.join(fixed_seq), ' '.join(fixed_seq_ops))
...
def repair_prog(actual_tokens, fix_seq_operations):
    ...
    # if "\'" in actual_tkns:
    #     actual_tkns = re.sub(re.compile(r"\'(\w*\s+)+\w*\'") , "\'string_with_spaces\'" , actual_tkns)
    # if "\"" in actual_tkns:
    #     actual_tkns = re.sub(re.compile(r"\"(\w*\s+)+\w*\"") , "\"string_with_spaces\"" , actual_tkns)
