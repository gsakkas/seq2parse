...
Our heuristic is to choose the longest possible partial parses to abstract our
erroneous programs and thus limiting the size of the program token sequence
...
\begin{figure}[t]
    \begin{ecode}
    [P = 100.0%] S -> Stmts_Or_Newlines Endmarker
    [P = 99.43%] Funcdef -> def Name Parameters : Suite
    [P = 99.29%] Suite -> Newline Indent Stmts_Or_Newlines Dedent
    [P = 38.72%] Stmts_Or_Newlines -> Stmt_Or_Newline
    [P = 61.28%] Stmts_Or_Newlines -> Stmt Stmts_Or_Newlines
    [P = 86.81%] Stmt -> Expr_Stmt
    [P =  0.17%] Stmt -> Pass_Stmt
    [P = 10.25%] Stmt -> Return_Stmt
    [P =  1.41%] Return_Stmt -> return
    [P = 98.59%] Return_Stmt -> return Arg_List
    \end{ecode}
