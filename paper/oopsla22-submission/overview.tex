...
production rules for it with the trained Transformer classifier. From the full
set of \emph{terminal} error rules, we rank them based on their predicted error
score from the classifier and return here the \emph{top 10} predictions.
Therefore, the predicted set of error rules is the following:
%
|E_number -> $\epsilon$|, |E_number -> Insert number|, |E_\n -> Insert \n|,
%
|E_( -> $\epsilon$|,\linebreak |E_return -> $\epsilon$|, |Token -> )|,
%
|Token -> +|, |Token -> :|, |Token -> name|, |Token -> number|.

We then parse the program in \autoref{fig:bad-prog} with the ECE-Parser and
those specific error rules to generate a valid parse. We observe that it takes
our implementation (as we will show later in depth) around \emph{2 seconds} to
generate a valid parse and suggest the actual user repair! On the other hand,
when we use the ECE-Parser with the full set of error rules it takes \emph{2
minutes and 55 seconds} to generate a valid parse, which is, however, not the
expected user parse, but the one presented in \autoref{fig:adding-partial}.
