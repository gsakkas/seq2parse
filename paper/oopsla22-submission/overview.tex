...
programmer intended, as shown in the fix in \autoref{fig:fixed-prog}. This is a
...
\emph{grammar} $G$. A grammar has a starting symbol |S| and a set of
...
symbols. For example, the non-terminal |Stmt| defines all the possible Python
statements, \ie expressions (|ExprStmt|), return statements (|ReturnStmt|), \etc
...
is used to match the empty symbol and generate a parser that suggests that a
