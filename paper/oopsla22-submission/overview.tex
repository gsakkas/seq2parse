...
in recent literature to automatically infer the intended parse tree for programs
that have \emph{syntax errors}. These dynamic programming parsers have been used
along side with \emph{error correcting production rules} \citep{Aho_1972} to
generate a valid parse by making minimal edits to the original program. As
mentioned before, EC-Parsers are very inefficient for larger real-world
...
original Earley parsing is a fairly efficient parsing algorithm
\citep{Earley_1970} based on dynamic programming.

\mypara{Solution: Train Classifier to Predict Error Rules}
As we show later in this section, not every erroneous program needs all possible
error production rules to be parsed, as the original algorithm \citep{Aho_1972}
suggests. In this work, we present a novel strategy to selecting just a small
...
are built top down. An (Earley) parser accepts programs that belong into a
...
that a Python program consists of and are here presented in all lowercase
letters. Symbols that start with a capital letter here represent
