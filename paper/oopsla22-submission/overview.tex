...
production rules. 
% as the original algorithm \citep{Aho_1972} suggests. 
...
ECE-Parser considers: \emph{insertion}, \emph{deletion}, and \emph{replacement} errors.
...
In this section we describe considerations that arise when using ECE-Parser for Python 
directly. 
\autoref{fig:partial-parse-tree-2} presents a \emph{partial parse}
...
We need to work on programs that have parse errors, which means that we can not generate a
parse tree for them to perform the usual of analyses associated with 
automated program repair \citep{Sakkas_2020,
Martinez_2013,Gulwani_2018, Wang_2018} or program analysis in general.
%
...
sequence is appropriate for training our sequence classifier models, but it 
can be large or contain irrelevant context for repairing
the parse error at hand. 
...
two suggested repairs in \autoref{fig:two-partial-parses}: if we delete
...
program, the first one matching \autoref{fig:partial-parse-tree-2}
and the second one not shown here. To chose between partial parses to
abstract the program around the parse error, we propose the use of
...
be ambiguous. Earley parsing is a dynamic programming technique
and internally keeps all possible parses for a program token sequence. 
Probabilistic Context-Free Grammars (PCFGs) have been used in previous work
...
repair in \autoref{fig:fixed-prog}! On the other hand, when we use a
baseline ECE-Parser with the full set of error rules it takes \emph{2 minutes and 55
seconds} to generate a valid parse, which is, additionally, not the expected user
parse, but the one presented in \autoref{fig:adding-partial}. These
