...
One approach to make erroneous programs such as in \autoref{fig:bad-prog} parse
is to use \emph{Error Correcting Parsers} (EC-Parsers) \citep{Aho_1972}.
EC-Parsers are based on \emph{dynamic programming algorithms} and have been used
in recent literature to automatically infer the intended parse tree for a
program that doesn't belong to a certain programming language, \ie a program
that has a syntax error. These dynamic programming parsers have been used along
side with \emph{error correcting production rules} \citep{Aho_1972} to generate
a valid parse by making minimal edits to the original program. As mentioned
before, EC-Parsers are very inefficient for larger real-world grammars, since it
requires the addition of a large number of error production rules, making them
impractical for real-world applications. In contrast, the original Earley
parsing is a fairly efficient parsing algorithm \citep{Earley_1970} based on
dynamic programming.

\mypara{Solution: Train Classifier to Predict Error Rules}
As we show later in this section, we observe that not every erroneous program
needs all possible error production rules to be parsed, as the original
algorithm \citep{Aho_1972} suggests. In this work, we present a novel strategy
to selecting just a small set of error production rules that can be used with an
updated \emph{Error Correcting Earley Parser} (ECE-Parser) to repair a program
with parse errors. In the remainder of this section, we give a high-level
overview of our approach by describing how to:
...
A \emph{parser} will hold some internal state of the partial parses it has
...
Sequence-to-sequence (seq2seq) architectures aim to transform a input sequence
...
repair in \autoref{fig:fixed-prog}! On the other hand, when we use the
ECE-Parser with the full set of error rules it takes \emph{2 minutes and 55
seconds} to generate a valid parse, which is, however, not the expected user
parse, but the one presented in \autoref{fig:adding-partial}. We observe here
the effectiveness of accurately \emph{predicting error rules using sequence
