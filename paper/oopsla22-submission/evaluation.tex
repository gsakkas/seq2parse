...
each class. Additionally, the are \emph{two input embedding layers} of a length
...
developers will consider at most five suggestions before falling back to manual
...
\textsc{MinimumCost} parser with 94.09\% parse accuracy and 93.42\% rare parse
  ...
  \begin{minipage}[c]{0.46\linewidth}
    ...
    \resizebox{\linewidth}{!}{
      \begin{tabular}{l||ccc}
        ...
        \textsc{Threshold}    & 94.09\%          & 93.42\%          & 21.19\% \\
      \end{tabular}
    }
...
repair rate of 81.59\% within 20 seconds and a median parse time of 2.5 seconds.
...
50 most popular ECE-Parser parses less programs until 28 seconds than the the 20
...
\begin{framed}
  ...
  majority of the test set in under 20 seconds with a median parse time of 2.5
  seconds.
\end{framed}
...
As \toolname is intended as an aid for programmers faced with parse errors, we
are also interested in the subjective human understanding of the quality and
...
(and thus repairs) are still helpful for debugging even when they differ from
...
the debugging hint was the human fix. Participants rated the quality and
...
Participants were unaware of if any given hint was generated by a human or
...
ratings from 39 valid participants (246 for human fixes and 281 for \toolname
repairs).
...
\begin{framed}
  \noindent Even when non-equivalent to the user's fix, \toolname's repairs are still useful and helpful:
